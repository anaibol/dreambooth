...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        example["instance_masks"], example["instance_masked_images"] = generate_random_mask(
        ...
        if self.with_prior_preservation:
            ...
            example["class_masks"], example["class_masked_images"] = generate_random_mask(
...
def main(args):
    ...
    def collate_fn(examples):
        ...
        masked_image_values = [example["instance_masked_images"]
        ...
        if args.with_prior_preservation:
            ...
            masked_image_values += [example["class_masked_images"]
                                    for example in examples]

        pixel_values = torch.stack(pixel_values).to(
            memory_format=torch.contiguous_format).float()
        mask_values = torch.stack(mask_values).to(
            memory_format=torch.contiguous_format).float()
        masked_image_values = torch.stack(masked_image_values).to(
            memory_format=torch.contiguous_format).float()
        ...
        if accelerator.is_main_process:
            ...
            pipeline = StableDiffusionInpaintPipeline.from_pretrained(
                ...
                unet=accelerator.unwrap_model(
                    unet, keep_fp32_wrapper=True).to(torch.float16),
            ...
            with accelerator.accumulate(unet):
                ...
                with torch.no_grad():
                    if not args.not_cache_latents:
                        ...
                        masked_latent_dist = vae.encode(
                            batch["masked_image_values"].to(dtype=weight_dtype)).latent_dist
                    latents = latent_dist.sample() * 0.18215
                    masked_image_latents = masked_latent_dist.sample() * 0.18215
                    mask = F.interpolate(
                        batch["mask_values"], scale_factor=1 / 8)
